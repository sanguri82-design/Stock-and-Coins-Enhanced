 ...
 * Modified in 2026 by sanguri82-design.
 *
...
const StocksMenuButton = GObject.registerClass({
  GTypeName: 'StocksCoinsEnhanced_MenuButton'
  ...
  _init ({ settings, extensionObject }) {
    ...
    this.add_style_class_name('stocks-coins-enhanced')

    this.add_child(new MenuStockTicker(this._settings))

    const bin = new St.Widget({ style_class: 'stocks-coins-enhanced' })
    bin._delegate = this
...
export default class StocksAndCoinsEnhancedExtension extends Extension {
  enable () {
    clearCache()
    initTranslations(_)
